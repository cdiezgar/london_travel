...
async function loadUserTrips() {
    ...
    // --- NUEVO: COMPROBAR INVITACIONES PENDIENTES ---
    const { data: invitacionesPendientes } = await sb.from('invitaciones_viaje')
    ...
    if (invitacionesPendientes && invitacionesPendientes.length > 0) {
        ...
        return; // Detenemos la carga de botones hasta que el usuario responda
    }

    // --- 2. OBTENER MIS VIAJES (PROPIETARIO) ---
    ...
    if (errorMisViajes) {
        console.error("Error cargando viajes:", errorMisViajes);
        tripList.innerHTML = '<p class="text-red-600 font-bold text-sm">Maldición detectada al cargar los viajes.</p>';
        return;
    }

    // --- 3. OBTENER VIAJES COMPARTIDOS (INVITADO) ---
    const { data: invitacionesAceptadas } = await sb.from('invitaciones_viaje')
        .select(`viajes (*)`)
        .eq('usuario_invitado_id', user.id)
        .eq('estado', 'aceptada');
    ...
    if (todosLosViajes.length === 0) {
        ...
        todosLosViajes.forEach(viaje => {
            ...
            // Añadimos el nombre y la flechita
            btn.innerHTML = `
